...
public:
	StFDTD_TM(std::shared_ptr<TYPE::Field> field, std::shared_ptr<FazzyModel> model);
	~StFDTD_TM();
	bool calc();
	void field();
...
#pragma omp for
#endif
					 for (int i = 0; i < mField -> getNpx(); i++) for (int j = 0; j < mField->getNpy() - 1; j++)
						 HX(i, j, +1) = CHX(i, j) * HX(i, j, 0) - CHXLY(i, j) * (EZX(i, j + 1, +1) - EZX(i, j, +1) + EZY(i, j + 1, +1) - EZY(i, j, +1)); // Hxの計算 Hx(i, j+1/2) -> Hx[i,j]
...
#pragma omp for
	...
	for (int i = 0; i < mField->getNpx() - 1; i++)
		for (int j = 0; j < mField->getNpy(); j++)
			HY(i, j, +1) = CHY(i, j) * HY(i, j, 0) + CHYLX(i, j) * (EZX(i + 1, j, +1) - EZX(i, j, +1) + EZY(i + 1, j, +1) - EZY(i, j, +1)); // Hyの計算 Hy(i+1/2, j) -> Hy[i,j]
}
}
;
}
